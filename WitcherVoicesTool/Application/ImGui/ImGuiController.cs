// Code sample taken from https://github.com/ImGuiNET/ImGui.NET/blob/master/src/ImGui.NET.SampleProgram/ImGuiController.cs and modified
...
public class ImGuiController : IDisposable
{
    ...
    public ImGuiController(GraphicsDevice InGraphicsDevice, OutputDescription InOutputDescription, int InWidth, int InHeight)
    {
        ...
        CreateDeviceResources(GraphicsDevice, InOutputDescription);
    ...
    public void CreateDeviceResources(GraphicsDevice InGraphicsDevice, OutputDescription InOutputDescription)
    {
        GraphicsDevice = InGraphicsDevice;
        ...
        byte[] vertexShaderBytes = LoadEmbeddedShaderCode(GraphicsDevice.ResourceFactory, "imgui-vertex", ShaderStages.Vertex);
        byte[] fragmentShaderBytes =
            LoadEmbeddedShaderCode(GraphicsDevice.ResourceFactory, "imgui-frag", ShaderStages.Fragment);
        VertexShader = factory.CreateShader(new ShaderDescription(ShaderStages.Vertex, vertexShaderBytes,
            GraphicsDevice.BackendType == GraphicsBackend.Metal ? "VS" : "main"));
        FragmentShader = factory.CreateShader(new ShaderDescription(ShaderStages.Fragment, fragmentShaderBytes,
            GraphicsDevice.BackendType == GraphicsBackend.Metal ? "FS" : "main"));
        ...
        FontTextureResourceSet =
            factory.CreateResourceSet(new ResourceSetDescription(TextureLayout, FontTextureView));
    }
    ...
    public IntPtr GetOrCreateImGuiBinding(ResourceFactory factory, TextureView textureView)
    {
        if (!_setsByView.TryGetValue(textureView, out ResourceSetInfo rsi))
        {
            ResourceSet resourceSet =
                factory.CreateResourceSet(new ResourceSetDescription(TextureLayout, textureView));
    ...
    /// </summary>
    public void RecreateFontDeviceTexture(GraphicsDevice GraphicsDevice)
    {
        ...
        FontTexture = GraphicsDevice.ResourceFactory.CreateTexture(TextureDescription.Texture2D(
            (uint)width,
            (uint)height,
            1,
            1,
    ...
    /// </summary>
    public void Render(GraphicsDevice GraphicsDevice, CommandList cl)
    {
        if (bFrameBegun)
        {
            bFrameBegun = false;
            ImGui.Render();
            RenderImDrawData(ImGui.GetDrawData(), GraphicsDevice, cl);
        }
    }
    ...
    private void RenderImDrawData(ImDrawDataPtr draw_data, GraphicsDevice GraphicsDevice, CommandList cl)
    {
        ...
        if (totalVBSize > VertexBuffer!.SizeInBytes)
        {
            ...
            VertexBuffer = GraphicsDevice.ResourceFactory.CreateBuffer(new BufferDescription((uint)(totalVBSize * 1.5f),
        ...
        if (totalIBSize > IndexBuffer!.SizeInBytes)
        {
            ...
            IndexBuffer = GraphicsDevice.ResourceFactory.CreateBuffer(new BufferDescription((uint)(totalIBSize * 1.5f),
